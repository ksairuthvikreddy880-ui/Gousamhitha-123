-- ============================================
-- RECREATE VENDORS AND PRODUCTS TABLES
-- Run this after deleting both tables
-- ============================================

-- Step 1: Create vendors table first (products depends on it)
...
-- Step 2: Create products table (with foreign key to vendors)
...
-- Step 3: Recreate cart table (depends on products)
-- Note: Only create if users table exists, otherwise skip
DO $$
BEGIN
    ...
    ELSE
        RAISE NOTICE 'Users table does not exist, skipping cart table creation';
    END IF;
END $$;

-- Trigger for cart updated_at (only if cart exists)
DO $$
BEGIN
...
-- Step 4: Recreate order_items table (depends on products and orders)
DO $$
BEGIN
    ...
    ELSE
        RAISE NOTICE 'Orders table does not exist, skipping order_items table creation';
    END IF;
END $$;

-- Step 5: Insert sample vendors
...
-- Show sample data
SELECT 'Sample Products with Vendors:' as info;
SELECT 
